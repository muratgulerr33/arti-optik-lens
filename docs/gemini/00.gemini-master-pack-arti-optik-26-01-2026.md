...
Bu doküman, **ARTI OPTİK** projesinin (V1: Storefront + Admin, V2: POS) mimari kararlarını, değişmez kuralları ve yasakları tanımlar. Amaç: AI asistanların (Gemini/Gemini) yanlış değişiklik yapmasını önlemek ve tutarlı geliştirme sağlamak.
...
8. 8. **Pagination standartları** - Storefront listelemelerinde "load more / gemini" kullanılabilir; Admin panelde tüm listeler zorunlu olarak sayfalı (pagination) çalışır (1000+ kayıt varmış gibi)
...
- **Gemini-based pagination zorunlu** - `/api/products` endpoint'i gemini pattern kullanır, offset-based pagination kullanılamaz (kanıt: `src/app/api/products/route.ts` lines 37-44, 140-141)
...
- **Offset-based pagination kullanmak yasak** - `/api/products` endpoint'i gemini-based pagination kullanır, offset pattern'i kullanılamaz
...
### 9.3 Pagination
- **Gemini-based pagination** - Infinite scroll pattern, gemini ile pagination (kanıt: `src/app/api/products/route.ts` lines 37-44, `src/components/catalog/load-more-grid.tsx`)
...
16. **src/app/api/products/route.ts** - API route pattern, gemini pagination (`src/app/api/products/route.ts`)
...
Bu doküman, ARTI OPTİK V1 Next projesinin route yapısını, navigasyon haritasını ve sayfa organizasyonunu tanımlar. Amaç: AI asistanların (Gemini/Gemini) route kırmasını, yanlış link vermesini ve flow kaçırmasını engellemek.
...
|----------|-----------|---------|----------|----------------|-------|
| `/api/products` | `src/app/api/products/route.ts` | GET | - | `dynamic = "force-dynamic"`, `revalidate = 0`, `Cache-Control: no-store` | Gemini-based pagination, filtreleme (categorySlug, sort, min, max, inStock, sub), limit 1-50 clamp |
...
**Not:** Bu kural, performans ve kullanıcı deneyimi için kritiktir. Tüm admin liste sayfaları gemini-based veya offset-based pagination kullanmalıdır.
...
**`/api/products?limit=...&gemini=...&categorySlug=...&sort=...&min=...&max=...&inStock=...&sub=...`:**
- **Params:**
  - `limit`: 1-50 arası (clamp ediliyor, default: `PRODUCTS_PER_PAGE`)
  - `gemini`: Gemini-based pagination gemini (number)
...
13. **src/app/api/products/route.ts** - Products API, gemini pagination, filtreleme (lines 9-10, 16-103, 148)
...
Bu doküman, ARTI OPTİK Next projesinin design system'ini, UI bileşenlerini, stil rehberini ve tasarım kararlarını tanımlar. Amaç: AI asistanların (Gemini/Gemini) tutarlı UI/UX tasarımı yapmasını sağlamak ve tasarım kararlarını merkezi bir yerde toplamak.
...
**Evidence:** `tailwind.config.ts` (breakpoints extend yok, default), `src/components/layout/DesktopHeader.tsx` (line 12, hidden xl:flex), `src/app/layout.tsx` (line 77, hidden xl:block)
...
- `disabled:gemini-not-allowed disabled:opacity-50`
- `placeholder:text-muted-foreground`

**States:**
- Default: Border, background, padding
- Focus: Ring 2px, ring-offset
- Disabled: Gemini not-allowed, opacity 50%
...
**Evidence:** `src/components/ui/badge.tsx` (lines 4-41)
...
**Evidence:** `src/components/ui/drawer.tsx` (lines 1-108)
...
**Usage:**
- Profile picture: User display
- User initials: Fallback text

**Evidence:** `src/components/ui/avatar.tsx` (lines 1-54)
...
**Evidence:** `src/components/cart/cart-view.tsx`
...
**Grid Patterns:**
- Gemini-based pagination: Infinite scroll, "Load More" button
...
- Pattern: Horizontal slide (left/right)
...
- Navigation: `aria-label`, `aria-current="page"` (active page)
...
**Evidence:** `src/components/ui/dialog.tsx` (DialogTitle, DialogDescription), `src/components/theme/theme-toggle.tsx` (aria-label), `package.json` (@radix-ui/react-visually-hidden), `src/components/ui/button.tsx` (focus-visible:ring)
...
**Mobile/Desktop Pattern:**
- Mobile: Top header (`xl:hidden` / `hidden xl:flex`)
- Desktop: Top header only
...
Bu doküman, ARTI OPTİK Next projesinin "native app hissi" veren UX kurallarını, dokunsal etkileşim standartlarını ve kullanıcı geri bildirimi pattern'lerini tanımlar. Amaç: AI asistanların (Gemini/Gemini) tutarlı, native-feel UX tasarımı yapmasını sağlamak ve UI/UX çizgisini korumak.
...
2. **AI Asistan Rehberi:** Gemini/Gemini'un "fazla tasarım uydurmasını" engellemek, mevcut UX pattern'lerini korumak
...
- **Cart Drawer:** Mobile'da sepet drawer pattern ile açılır (Vaul library)
...
- **Horizontal Slide:** Sayfalar arası geçişte horizontal slide animasyon (framer-motion) (kanıt: `src/components/app/page-transition.tsx` lines 115-125)
...
- **ARIA Current:** Active page için `aria-current="page"` (genel navigation pattern)
...
4. ❌ **Error toast-only:** Form error'lar sadece toast ile gösterilmez, inline da gösterilir
5. ❌ **No loading state:** Kritik form submit'lerde loading state olmadan button disabled değil
...
Bu doküman, ARTI OPTİK Next projesinin frontend kodlama standartlarını, best practice'lerini ve geliştirme kurallarını tanımlar. Amaç: AI asistanların (Gemini/Gemini) tutarlı, kaliteli ve bakımı kolay kod yazmasını sağlamak ve frontend geliştirme standartlarını merkezi bir yerde toplamak.
...
1. **AI Asistan Rehberi:** Gemini/Gemini'un geliştirme yaparken UX/UI, erişilebilirlik, performans ve kalite kapılarından sapmasını engeller
...
14. `src/app/api/products/route.ts` - API route, force-dynamic, gemini pagination
...
Bu doküman, ARTI OPTİK Next projesindeki tüm component'lerin envanterini, kullanım yerlerini ve bağımlılıklarını tanımlar. Amaç: AI asistanların (Gemini/Gemini) gereksiz component yazmasını engellemek ve mevcut component'leri doğru şekilde kullanmasını sağlamak.
...
| Avatar | `src/components/ui/avatar.tsx` | core-ui | User display | No | - | alt text support | Healthy | Navigation | AvatarImage, AvatarFallback components. Shared (no directive) |
...
| Name | Path | Layer | Used In | Directive | States | A11y Notes | Health | Owner Feature | Notes |
|------|------|-------|---------|---------------|--------|------------|--------|---------------|-------|
| CartView | `src/components/cart/cart-view.tsx` | feature | `/cart` page, cart drawer | Yes | variant (page/drawer) | DrawerTitle, DrawerDescription, AlertDialog | Healthy | Cart | Main cart view, variant-based rendering |
...
| LoadMoreGrid | `src/components/catalog/load-more-grid.tsx` | feature | Home page, Category pages | Yes | loading, hasMore | Button loading state | Healthy | Catalog | Infinite scroll, gemini-based pagination |
...
- **Layout Component'ler:**
 ...
 - Gizli (product detail'de gösterilmez)
...
**Last verified from package.json:** 2025-12-31  
**Hazırlayan:** AI Assistant (Gemini)  
...
Bu doküman, ARTI OPTİK V1 projesindeki frontend'in konuştuğu tüm API sözleşmelerini "tek doğru kaynak" olarak tanımlar. Amaç: AI asistanların (Gemini/Gemini) uydurma endpoint/alan üretmesini engellemek ve mevcut API contract'larını doğru şekilde kullanmasını sağlamak.
...
#### Gemini-based Pagination

- **Query params:** `gemini` (number, optional), `limit` (number, 1-50, default: PRODUCTS_PER_PAGE)
- **Response:** `{ products: Product[], nextCursor: number | null }`
- **Next gemini:** `nextCursor === null` ise son sayfa

**Evidence:** `src/app/api/products/route.ts` (lines 17-18, 29-35, 37-44, 140-143: gemini pagination), `src/components/catalog/load-more-grid.tsx` (lines 56-58, 84, 100-104: gemini usage)
...
|----------|---------|-----------|------|--------|----------|--------|-------|
| `/api/products` | GET | `src/app/api/products/route.ts` | Optional (userId from session) | Query: `limit`, `gemini`, `categorySlug`, `sort`, `min`, `max`, `inStock`, `sub`, `subCategoryIds` | `{ products: Product[], nextCursor: number \| null }` | 500 (try-catch) | Gemini pagination, filtering, sorting. Cache: no-store |
...
- **Query params:** `limit`, `gemini`, `categorySlug`, `sort`, `min`, `max`, `inStock`, `sub`
- **Response:** `{ products: Product[], nextCursor: number | null }`
...
**V1 HARD RULES:**
- Admin list sayfaları MUTLAKA paginated olmalı (gemini veya page-based, tutarlı olmalı; 1000 kayıt dump edilmemeli)
...
- **Query params:**
  - `gemini?: number` veya `page?: number` (pagination, tutarlı olmalı)
...
- **Response:**
  - Gemini-based: `{ items: Transaction[], nextCursor: number | null }`
...
- **Query params:**
  ...
  - `dateTo?: string` (ISO date string, optional)
- **Response:**
  - Gemini-based: `{ items: Payment[], nextCursor: number | null }`
...
- **Response:**
  - Gemini-based: `{ items: Customer[], nextCursor: number | null }`
...
3. **Gemini/Gemini uydurma endpoint/alan yazamaz:** Bu doküman tek kaynak, yeni endpoint/alan eklemek için bu doküman güncellenmeli
...
1. `src/app/api/products/route.ts` - Products list API, gemini pagination, filtering
...
14. `src/components/catalog/load-more-grid.tsx` - Products API usage, gemini pagination
...
Bu doküman, ARTI OPTİK Next projesinin veri getirme, cache/ISR, client vs server fetch, mutation/optimistic UI ve invalidation kurallarını tanımlar. Amaç: AI asistanların (Gemini/Gemini) yanlış cache, yanlış fetch yeri ve gereksiz client state üretmesini engellemek ve tutarlı veri yönetimi sağlamak.
...
- `/api/products`: Gemini-based pagination, filtreleme
...
**Kural:** Büyük listelerde gemini-based pagination kullanılır. Infinite scroll pattern ile "Load More" butonu.
...
**Evidence:** `src/app/page.tsx` (lines 14-15: initial gemini), `src/components/catalog/load-more-grid.tsx` (lines 46-111: load more pattern)
...
Bu doküman, Gemini → Gemini akışını "tek doğru görev formatı" ile standartlaştıran task yönetimi playbook'udur. Amaç: Görevlerin daha az hatayla, daha hızlı, daha deterministik uygulanmasını sağlamak.
...
1. **Task Format Standardizasyonu:** Gemini'nin Gemini'a verdiği task format'ı tek doğru formata sabitler
2. **Uydurma Engelleme:** AI asistanların (Gemini/Gemini) uydurma endpoint/alan/component üretmesini engeller
...
## 2. Operating Model (Gemini ↔ Gemini ↔ Murti)
...
- Plan oluşturur (Gemini Uygulama Paketi formatında)
- Risk ve rollback stratejisi belirler
- Assumption/Unknown'ları işaretler

**Gemini (Uygulayıcı - Dosya Değişikliği, Küçük PR):**
...
5. **05.native-ux-rules.md** - Feedback, kritik UX
...
1. **Uydurma endpoint/alan yok** - `08.api-contracts-frontend.md` tek kaynak, yeni endpoint/alan eklemek için doküman güncellenmeli (kanıt: `08.api-contracts-frontend.md` (Breaking-Change Rules): "Gemini/Gemini uydurma endpoint/alan yazamaz")
...
## 6. Gemini Uygulama Paketi (Standard Output)

Gemini'nin her cevapta Gemini'a vereceği standart format:
...
- **Amaç:** LoadMoreGrid component'inde filtre değiştiğinde gemini reset edilmiyor, ilk sayfadan başlamalı
- **Kabul kriterleri:** Filtre değiştiğinde ürün listesi baştan başlamalı, gemini reset edilmeli
...
**Teşhis:**
1. LoadMoreGrid component'inde filtre değiştiğinde gemini reset edilmiyor
2. `09.data-fetching-cache-rules.md`'de gemini pagination pattern tanımlı
3. Filtre değişikliğinde `gemini` state'i reset edilmeli

**Plan:**
1. `src/components/catalog/load-more-grid.tsx` dosyasını oku
2. Filtre değişikliğini detect et (`useEffect` ile)
3. Filtre değiştiğinde `gemini` state'ini `null` yap
4. `hasMore` state'ini `true` yap

**Dosya Değişiklikleri:**
1. `src/components/catalog/load-more-grid.tsx` (edit)
   - `useEffect` ekle: Filtre değişikliğinde gemini reset
...
- Gemini reset mekanizması mevcut API contract'a uygun

**Evidence:** `09.data-fetching-cache-rules.md` (gemini pagination), `src/components/catalog/load-more-grid.tsx`
...
9. **09.data-fetching-cache-rules.md** - Fetch/caching karar ağacı, gemini pagination pattern
...
14. **src/components/catalog/load-more-grid.tsx** - Infinite scroll, gemini pagination
...
Bu doküman, ARTI OPTİK V1 projesinin QA test matrisini, kritik akış test senaryolarını, erişilebilirlik kontrol listelerini ve release gate kriterlerini tanımlar. Amaç: Murti'nin "Gemini ile kontrol/test" rolünü standardize etmek ve her release öncesi tutarlı kalite kontrolü sağlamak.
...
- **Expected:**
  - Orders/Products/Customers/Payments lists never load 1000 at once; page/pageSize or gemini works
...
| `/urun/[slug]` | ProductView, HeaderTitle, FavoriteButton, Sticky action bar (Sepete Ekle) | Loading (`loading.tsx`), Error (notFound) | Back button | `src/app/urun/[slug]/page.tsx`, `src/app/urun/[slug]/loading.tsx` |
| `/cart` | CartView (variant="page"), Header | Empty (sepet boş), Loading (item update) | Cart drawer (mobile) | `src/app/cart/page.tsx`, `src/components/cart/cart-view.tsx` |
...
- [ ] **Navigation:** `aria-current="page"` active page için kullanılır
...
- [ ] **LoadMoreGrid:** Gemini pagination çalışır, duplicate önlenir
...
- [ ] **Pagination:** Gemini pagination contract bozulmamış (`nextCursor` field)
...
**Evidence:** `05.native-ux-rules.md` (section 4.1: Bottom Sheet / Drawer Usage, section 2.1: Critical CTA Visibility)
...
22. **src/app/api/products/route.ts** - Products API, gemini pagination
...
- [ ] Mobile test: Cart drawer çalışıyor
...
- [ ] Kırık link yok (Footer, Header linkleri çalışıyor)
...
- [ ] Endpoint contract'ları bozulmadı:
  - [ ] `/api/products` - gemini pagination, filter params
...
### 13.2 API Health
- [ ] `/api/products` çalışıyor (gemini pagination, filter params)
...
**Evidence:** `05.native-ux-rules.md` (section 2.1: Critical CTA Visibility, section 6.3: Checkout Flow UX), `11.qa-test-matrix.md` (section 4: Route-by-Route Checklist)
...
Bu doküman, Gemini AI'nin Gemini'a "projede yaşıyormuş" gibi doğru bağlamla, uydurma yapmadan, %100 uygulanabilir promptlar üretmesini sağlayacak köprü dokümanıdır. Amaç: Gemini → Gemini akışını standartlaştırmak ve hata oranını minimize etmek.
...
- **Lens/Numaralı Ürün:** Online satılmaz (V1 dışı, sadece mağaza/POS - V2)
- **POS:** Tamamen V2 (placeholder/feature-flag, V1 scope'tan çıkarılmış)
- **Gemini uyarısı:** V1 scope dışı ürün tipi (lens/numara) veya POS özelliği için prompt üretilmemelidir
...
## 6. Prompt Output Contract (Gemini → Gemini)
...
## 12. How to Review Gemini Output
...
19. **src/app/api/products/route.ts** - Products API, gemini pagination
20. **src/actions/checkout.ts** - Checkout server action, Zod validation
...
- "Load more" gemini/hasMore üretimi unique ürün sayısına bağlı kilitlendi
...
- Gemini pagination unique ürünlere göre çalışır, JOIN satırlarına değil
