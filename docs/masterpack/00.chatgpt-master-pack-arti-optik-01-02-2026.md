...
**Source of Truth:** `docs/forensics/FAVORITES_V1_LOCK.md`. Aşağıdaki metin masterpack içine gömülü kopyadır; tek gerçek kaynak lock doc'tur.

**UI (Heart icon) standardı (FINAL):**
- Arka plan/daire yok, `bg-transparent`
...
- Touch target: 44×44; Light/Dark aynı görünüm; Pending: buton disabled + spinner (outline ile aynı ton)
...
- Guest kalbe basınca: `/auth/login?callbackUrl=...`; sessionStorage `ao:wishlist:intent` (productId, returnTo, scrollY, ts)
- Login sonrası: aynı sayfaya dön + scroll restore + otomatik add + toast + kalp dolu + wishlist'te görünür; callbackUrl sadece internal path

**Wishlist sayfa davranışı:**
- Remove animasyonu: fade/scale-out sonra listeden düşme; toast add/remove
- Canonical route: `/hesabim/wishlist` → `redirect("/account/wishlist")`

**Concurrency / Spam koruması:**
- FavoritesProvider inFlight lock: aynı productId'ye eşzamanlı istek engeli

**Manual QA checklist (V1):** Guest→login→geri dön + scroll + toast + kalp dolu; auth user spam tek istek; wishlist remove animasyon + toast; refresh sonrası favoriler kalıcı.

**Backlog (V2) — V1'e GİRMEYECEK:** Undo toast, bulk remove, offline/guest local wishlist, analytics event, e2e stabilizasyon, skeleton/loading polish, accessibility audit.
...
- **Provider Hierarchy:** (sıra önemli)
  ...
  5. `HeaderProvider`
  6. `SearchProvider`
- **Components:** `Header`, `DesktopHeader`, `SearchOverlay`, `Footer` (xl+), `Toaster` (Sonner)
...
### 5.3 Account Sidebar Navigation
...
- **Rule:** Yeni top-level sayfa açılırsa slug `RESERVED_SLUGS` array'ine eklenmeli (kanıt: `src/app/[slug]/page.tsx` lines 22-36, `src/components/layout/header.tsx` lines 13-35)

**Evidence:** `src/app/[slug]/page.tsx` (lines 22-36, 38-51), `src/components/layout/header.tsx` (lines 13-35)
...
21. **src/components/account/AccountSidebar.tsx** - Account sidebar nav, menu items (lines 6-12)
22. **src/components/admin/admin-sidebar.tsx** - Admin sidebar nav, menu items (lines 8-30)
23. **src/components/layout/header.tsx** - Header, RESERVED_SLUGS array (lines 13-35)
24. **src/components/app/Footer.tsx** - Footer nav, broken link `/sss` (line 61)
25. **package.json** - Next.js version, dependencies (next@16.1.1)
...
- Desktop: `xl:` ve üzeri
- Desktop header: `hidden xl:flex` (mobile'da header, desktop'ta DesktopHeader)

**Evidence:** `tailwind.config.ts` (breakpoints extend yok, default), `src/components/layout/DesktopHeader.tsx` (line 12, hidden xl:flex), `src/app/layout.tsx` (layout)
...
**Evidence:** `src/components/ui/badge.tsx` (lines 4-41), header cart icon (badge count)
...
**Evidence:** `src/components/ui/drawer.tsx` (lines 1-108), cart drawer (header’dan tetiklenir)
...
**Usage:**
- Profile picture: Header / account
- User initials: Fallback text

**Evidence:** `src/components/ui/avatar.tsx` (lines 1-54), header user avatar
...
Ürün kartı. Aspect ratio `4/5`, rounded-xl. Image: Hover'da gallery rotation (1s interval). Press-preview: Mobilde 150ms long-press ile 2. resim gösterimi. Quick add: Sağ alt köşe, Plus icon. Favorite: Sağ üst, FavoriteButton (V1 lock: bg-transparent; outline text-zinc-900/25 + drop-shadow 0.28; filled fill-red-500; 44×44; press active scale duration-100 scale-[0.92]). Title: ALL CAPS ise lowercase + capitalize. Price: Bold, formatPrice.
...
**Evidence:** `src/components/cart/cart-view.tsx` (drawer variant, page variant)
...
- Cart: Mobile'de drawer (header’dan tetiklenir), desktop'ta page
- Active state: Visual indicator (color change) where applicable

**Evidence:** `src/components/layout/header.tsx` (lines 46-238), `src/components/layout/DesktopHeader.tsx` (lines 10-65)
...
- Quick actions: Favorite (sağ üst, V1 lock: Heart outline text-zinc-900/25 + drop-shadow; filled red; 44×44; press active scale), Quick add (sağ alt)
...
- Active scale: `active:scale-[0.98]` veya `active:scale-95` (button feedback). İstisna: FavoriteButton V1 lock `active:scale-[0.92]` (FAVORITES_V1_LOCK.md).
- Touch callout disable: `[-webkit-touch-callout:none]` (image area)
- Safe area: `env(safe-area-inset-bottom)` (notch support)

**Mobile Patterns:**
- Cart: Mobile'da drawer (header’dan), desktop’ta sayfa
...
**Evidence:** `src/components/product/product-card.tsx` (lines 121-170, press preview), `src/components/app/page-transition.tsx` (line 72, container padding), `tailwind.config.ts` (breakpoints)
...
- Button press: `active:scale-95` (transform). İstisna: FavoriteButton V1 lock `active:scale-[0.92]` (FAVORITES_V1_LOCK.md).
...
**Evidence:** `src/components/ui/dialog.tsx` (DialogTitle, DialogDescription), `src/components/cart/add-to-cart-bar.tsx` (aria-label), `src/components/theme/theme-toggle.tsx` (aria-label), `package.json` (@radix-ui/react-visually-hidden), `src/components/ui/button.tsx` (focus-visible:ring)
...
**Evidence:** `src/app/globals.css` (primary #ff2357), `src/components/ui/button.tsx` (CVA pattern), `src/components/theme/theme-provider.tsx` (next-themes)
...
**Touch Interactions:**
- Active scale: `active:scale-[0.98]` veya `active:scale-95`. İstisna: FavoriteButton V1 lock `active:scale-[0.92]` (FAVORITES_V1_LOCK.md).
...
17. `src/components/app/page-transition.tsx` - Page transition animations (framer-motion)
...
**Evidence:** `src/components/product/product-view.tsx` (toast + button state), `src/components/app/page-transition.tsx` (reduced motion)
...
- **Safe Area Support:** iOS home indicator alanına uyum zorunlu - `pb-[max(1rem,env(safe-area-inset-bottom))]` veya `pb-[env(safe-area-inset-bottom)]` (kanıt: `src/components/product/product-view.tsx` line 268)

**Evidence:** `src/components/product/product-view.tsx` (lines 268-308, sticky bar), `src/components/product/product-detail-page.tsx` (lines 361-393)

### 2.2 Touch Target Sizes
- **Minimum Size:** Yeni/yenilenen interactive elementler minimum `h-11` (44px) veya `min-h-11` (kanıt: `src/components/ui/button.tsx` line 24)
...
**Evidence:** `src/components/product/product-view.tsx` (h-12), `src/components/ui/button.tsx` (h-11 default), `src/components/theme/theme-toggle.tsx` (h-11 w-11)
...
**Evidence:** `src/components/product/product-view.tsx` (sticky bottom)
...
- **Secondary Actions:** Favori butonu (FavoriteButton), theme toggle gibi ikincil aksiyonlar daha küçük, daha az vurgulu. FavoriteButton V1 lock'a uyar: bg-transparent, outline text-zinc-900/25 + drop-shadow, filled red, 44×44, press active scale (kanıt: `src/components/product/product-view.tsx` lines 277-283, `docs/forensics/FAVORITES_V1_LOCK.md`)
...
- **Active Scale:** Tüm butonlarda `active:scale-[0.98]` veya `active:scale-95` - dokunsal feedback (kanıt: `src/components/product/product-view.tsx` line 288, `src/components/ui/button.tsx` line 7). **İstisna:** FavoriteButton V1 lock'ta `active:scale-[0.92]` + `duration-100` (docs/forensics/FAVORITES_V1_LOCK.md).
...
**UI (Heart) — V1 lock (FINAL):** FavoriteButton görünümü V1 SCOPE LOCK → V1 LOCK — Favorites/Wishlist ile sabittir: bg-transparent; outline = fill-transparent + text-zinc-900/25 + drop-shadow-[0_1px_1px_rgba(0,0,0,0.28)]; filled = fill-red-500 text-red-500; 44×44; tema bağımsız; pending disabled+spinner; press = active scale (duration-100, scale-[0.92]). Detay: `docs/forensics/FAVORITES_V1_LOCK.md`.
...
- **Guest Gate:** Misafir kullanıcı favoriye basarsa auth'a yönlendirilir (login/signup entrypoint'leri dead-end olmayacak). Guest flow: kalbe basınca `/auth/login?callbackUrl=...`; sessionStorage `ao:wishlist:intent`; login sonrası dönüş + scroll + otomatik add + toast + kalp dolu.
...
- **Concurrency:** FavoritesProvider inFlight lock ile aynı productId'ye eşzamanlı istek engeli.

**LOCK RULE:** Client UI layer `@/actions/favorites` import edemez; yalnızca `FavoritesProvider` server action çağırır.

**Evidence:**
- `src/components/favorites/favorite-button.tsx` (UI tek kapı; V1 lock UI token'ına uyar)
- `src/components/favorites/favorites-provider.tsx` (toggle + optimistic + state + inFlight lock)
...
### 4.1 Bottom Sheet / Drawer Usage
- **Cart Drawer:** Mobile'da sepet drawer pattern ile açılır (Vaul library) (header’dan tetiklenir)
...
- **Pattern:** Bottom-up slide animasyon, `rounded-t-2xl` top corners, backdrop blur, `max-h-[85dvh]` (kanıt: `src/components/ui/drawer.tsx`, `src/components/hub/category-quick-look-sheet.tsx` line 100)

**Evidence:** `src/components/ui/drawer.tsx` (drawer component), `src/components/account/address-form.tsx` (address form drawer), `src/components/hub/category-quick-look-sheet.tsx` (quick look sheet), `docs/locks/hub-native-navigation-lock.md` (section 9: Drawer Rules)
...
- **Active Scale:** `active:scale-[0.98]` veya `active:scale-95` - dokunsal feedback (kanıt: `src/components/product/product-card.tsx` line 202, `src/components/product/product-view.tsx` line 288). **İstisna:** FavoriteButton V1 lock `active:scale-[0.92]` (FAVORITES_V1_LOCK.md).
...
- **Scroll Intent:** Sayfa geçişlerinde scroll restore - `page-transition.tsx` (kanıt: `src/components/app/page-transition.tsx` lines 46-63, `useLayoutEffect`)

**Evidence:** `src/components/app/page-transition.tsx` (lines 46-63, scroll restore)
...
- **Focus Ring:** Tüm interactive elementlerde `focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2` (kanıt: `src/components/ui/button.tsx` line 7)
...
**Evidence:** `src/components/ui/button.tsx` (focus-visible:ring), `src/components/ui/dialog.tsx` (Radix UI)
...
- **ARIA Labels:** Icon-only butonlarda `aria-label` (kanıt: `src/components/cart/add-to-cart-bar.tsx` line 47)
- **Semantic HTML:** Native elements (button, nav, form) kullanılıyor

**Evidence:** `src/components/cart/add-to-cart-bar.tsx` (aria-label)
...
2. ❌ **Küçük touch target:** 44px'den küçük interactive element yok
3. ❌ **Aşırı animasyon:** Sadece gerekli animasyonlar (page transitions, button feedback)
4. ❌ **Scroll restore yok:** Sayfa geçişlerinde scroll pozisyonu kaybolmamalı
5. ❌ **Error toast-only:** Form error'lar sadece toast ile gösterilmez, inline da gösterilir
6. ❌ **No loading state:** Kritik form submit'lerde loading state olmadan button disabled değil

**Evidence:** `src/components/product/product-view.tsx` (DO examples), `src/app/checkout/page.tsx` (DO examples)

---

## 12. Exceptions (Allowed Deviations)

### 12.1 AddToCartBar - Shorter Timeout
...
### 12.2 Checkout - No Toast on Error
...
- **Favorites Empty:** Favoriler boşsa empty state gösterilir; metin/CTA FAVORITES_V1_LOCK.md Wishlist sayfa davranışı ile uyumlu olmalı (örn. "Henüz favorin yok" + "Kategorilere Git").
- **Orders Empty:** Sipariş listesi boşsa empty state görülmedi

**Evidence:** Unknown (empty state components görülmedi), `docs/forensics/FAVORITES_V1_LOCK.md` (Wishlist sayfa)
...
2. **src/app/layout.tsx** - Toaster mount, position config (line 82)
3. **src/components/cart/add-to-cart-bar.tsx** - Add to cart bar, shorter timeout (lines 24-38, 46-49)
4. **src/app/checkout/page.tsx** - Checkout flow, form validation, error display, loading state (lines 25-309)
5. **src/components/app/page-transition.tsx** - Page transitions, reduced motion, scroll restore (lines 82-160)
6. **src/components/app/tab-scroll.ts** - Tab scroll preservation (scroll save/read functions)
7. **src/components/product/product-card.tsx** - Press preview, touch interactions (lines 12-15, 121-170)
8. **src/components/ui/skeleton.tsx** - Skeleton loading component
...
22. **src/app/not-found.tsx** - Not found page (error handling reference)
...
| Avatar | `src/components/ui/avatar.tsx` | core-ui | Header profile, user display | No | - | alt text support | Healthy | Navigation | AvatarImage, AvatarFallback components. Shared (no directive) |
...
| ProductCard (product) | `src/components/product/product-card.tsx` | feature | Catalog ProductCard wrapper | Yes | - | Link, image alt, FavoriteButton (V1 lock) | Healthy | Product | Core product card, FavoriteButton integration (FAVORITES_V1_LOCK.md) |
...
| Name | Path | Layer | Used In | Directive | States | A11y Notes | Health | Owner Feature | Notes |
|------|------|-------|---------|---------------|--------|------------|--------|---------------|-------|
| CartView | `src/components/cart/cart-view.tsx` | feature | `/cart` page, cart drawer (header) | Yes | variant (page/drawer) | DrawerTitle, DrawerDescription, AlertDialog | Healthy | Cart | Main cart view, variant-based rendering |
...
| FavoriteButton | `src/components/favorites/favorite-button.tsx` | component | ProductCard, ProductDetailSticky, Wishlist | Yes | - | - | Locked | Favorites | Must call `useFavorites().toggle`; must NOT import server actions. V1 lock UI: bg-transparent, outline text-zinc-900/25 + drop-shadow 0.28, filled red, 44×44, press active scale (FAVORITES_V1_LOCK.md) |
...
- **Ana Component'ler:**
  ...
  - `FavoriteButton` - Favori butonu (V1 lock: Heart outline text-zinc-900/25 + drop-shadow, filled red, 44×44, press active scale; FAVORITES_V1_LOCK.md)
- **Layout Component'ler:**
...
- **V1 Lock (UI + flow):** Tüm davranış ve görünüm `docs/forensics/FAVORITES_V1_LOCK.md` ile kilitlidir (UI Heart FINAL token, Guest flow, Wishlist sayfa, Concurrency, QA checklist, Backlog V2).
- **Server Actions:**
...
- **Auth:** Required (Unauthorized döner). Guest kalbe basınca `/auth/login?callbackUrl=...`; sessionStorage `ao:wishlist:intent`.
- **Usage:** `src/components/favorites/favorite-button.tsx` (V1 lock UI'ya uyar)

**Evidence:** `src/actions/favorites.ts` (lines 7-88), `src/components/favorites/favorite-button.tsx` (line 69: `toggleFavoriteAction`), `docs/forensics/FAVORITES_V1_LOCK.md`
...
18. `src/components/favorites/favorite-button.tsx` - Favorites UI tek kapı, V1 lock (FAVORITES_V1_LOCK.md) UI token + server action usage
...
5. **05.native-ux-rules.md** - Feedback, kritik UX, sticky action bar
...
- **Expected:**
  ...
  - Favoriler listesi gösterilir; canonical `/hesabim/wishlist` → redirect `/account/wishlist`
- **Evidence:** `03.routes-and-navigation-map.md` (section 6.3), `src/app/account/layout.tsx` (auth protection), `docs/forensics/FAVORITES_V1_LOCK.md` (Wishlist sayfa davranışı)

### SMK-05: Product Detail → Add to Favorites
- **Steps:**
  1. `/urun/[slug]` - Ürün detay sayfası
  2. Favorite button'a tıklanır (V1 lock: Heart UI, press active scale)
- **Expected:**
  ...
  - Guest: Kalbe basınca login'e yönlendirme, sessionStorage `ao:wishlist:intent`, login sonrası dönüş + scroll + otomatik add (V1 lock Guest flow)
- **Evidence:** `09.data-fetching-cache-rules.md` (section 8.2), `src/components/favorites/favorites-provider.tsx` (optimistic update pattern), `docs/forensics/FAVORITES_V1_LOCK.md` (Guest flow, Concurrency)
...
|-------|----------------------|------------------|------------------------|----------|
| `/` | Category grid, Product grid (LoadMoreGrid), Header, Footer | Loading (skeleton), Empty (kategori/ürün yoksa) | - | `src/app/page.tsx`, `03.routes-and-navigation-map.md` (section 2) |
...
| `/urun/[slug]` | ProductView, HeaderTitle, FavoriteButton, Sticky action bar (Sepete Ekle) | Loading (`loading.tsx`), Error (notFound) | Back button | `src/app/urun/[slug]/page.tsx`, `src/app/urun/[slug]/loading.tsx`, `05.native-ux-rules.md` |
| `/cart` | CartView (variant="page"), Header | Empty (sepet boş), Loading (item update) | Cart drawer (mobile, header’dan) | `src/app/cart/page.tsx`, `src/components/cart/cart-view.tsx` |
...
- [ ] **Empty wishlist:** "Henüz favorin yok" + "Kategorilere Git" button (Wishlist sayfa davranışı: FAVORITES_V1_LOCK.md)
...
**Evidence:** `05.native-ux-rules.md` (section 9.3), `src/components/cart/cart-icon-button.tsx`
...
**Evidence:** `05.native-ux-rules.md` (Critical CTA, Drawer)
...
**Evidence:** `05.native-ux-rules.md` (section 4: Drawer, Touch)
...
26. **src/components/favorites/wishlist-client.tsx** - Empty wishlist state, remove fade/scale-out (FAVORITES_V1_LOCK.md Wishlist sayfa)
...
**Evidence:** `03.routes-and-navigation-map.md` (section 5: Navigation Entry Points), `src/components/app/Footer.tsx`
...
**Evidence:** `05.native-ux-rules.md` (section 2.1: Critical CTA Visibility)
...
**Evidence:** `05.native-ux-rules.md` (section 2.1: Critical CTA, section 6.3: Checkout Flow UX), `11.qa-test-matrix.md` (section 4: Route-by-Route Checklist)
...
19. **src/app/checkout/page.tsx** - Checkout flow, form validation, error handling, loading state
20. **src/app/layout.tsx** - Toaster component, toast config (position: top-center)
21. **src/components/layout/header.tsx** - RESERVED_SLUGS array
22. **src/components/app/Footer.tsx** - Footer navigation links (broken link: /sss)
23. **src/app/[slug]/page.tsx** - Category page, RESERVED_SLUGS kontrolü, notFound()
24. **src/app/urun/[slug]/page.tsx** - Product detail page, notFound()
...
**Evidence:** `05.native-ux-rules.md` (section 2.1: Critical CTA Visibility, section 3.1: Critical Action Feedback Pattern)
...
### 12.4 PDP: Sticky Action Bar

- [ ] Sticky action bar görünür (`/urun/[slug]`)
