...
**Next.js:** 16.1.1 (çekirdek ile aynı)  
**React:** 19.2.3 (çekirdek ile aynı)
...
3. **Drizzle ORM + PostgreSQL** - ARTI OPTİK katalog modeli: Category hiyerarşisi (ltree), Product (base) + Variant (SKU) ayrımı, Variant attributes JSONB + GIN index yaklaşımı (Prisma yok)
...
- **Design tokens merkezi yönetilir** - Tema/token değerleri sadece "tema kararı" geldikten sonra kilitlenir. Tema kararı gelmeden AI asistan **token icat edemez** ve rastgele değiştiremez.
...
### 2.11 Commerce Locks (ARTI OPTİK)
- **Ödeme yöntemi tek:** Kapıda ödeme YOK. Checkout sadece **kredi kartı (PayTR)** ile çalışır.
- **Kargo:** **Yurtiçi Kargo** entegrasyonu + **sabit kargo ücreti** (hesaplama kuralı değişmez).
- **Ürün URL standardı:** Ürün detay yolu **/urun/[slug]** (çekirdek ile aynı).
- **Barkod zorunluluğu (V1 hedef):** Satılabilir birim (Variant/SKU) barkodsuz olmaz (POS ve stok senkronu için).

### 2.12 POS & Stock Locks (ARTI OPTİK)
- **POS ayrı program değil:** Admin panel içinde "POS Hızlı Satış" menüsüdür.
- **Hızlı satış akışı kilit:** Barkod/ürün kodu gir → **Enter** → varsayılan ödeme **Nakit** seçili → satış oluşur → stok düşer.
- **Ürün yoksa akış kilit:** "Ürün sistemde yok" → **[Ürün Ekle]** butonu → barkod alanı otomatik dolu gelir.
- **Stok düşümü atomik olmalı:** Aynı anda son ürün satışı gibi çakışmalarda **negatif stok oluşamaz**. Stok düşümü tek hamlede güvenli yapılır.
- **Tek stok kaynağı:** Mağaza + Online aynı stok havuzunu kullanır (POS satışı online stoktan da düşer).
- **İşlemi Geri Al zorunlu:** Yanlış POS satışı için "Ayarlar > İşlemi Geri Al" ekranı olacak:
  - İşlem logları (pagination)
  - İşlem detayı (satırları genişlet)
  - **[Bu İşlemi Geri Al]** + onay adımı
  - Geri alınan işlem "geri alındı" etiketiyle logda kalır (iz bırakır).
---
