...
Bu doküman, ARTI OPTİK V1 Next projesinin route yapısını, navigasyon haritasını ve sayfa organizasyonunu tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) route kırmasını, yanlış link vermesini ve flow kaçırmasını engellemek.
...
**Dynamic Routes:**
- `[slug]` - Kategori sayfaları için (`/[slug]/page.tsx`) - **Not:** Kategori listeleme URL pattern'i TBD (karar sonra). Placeholder: `/<...categoryPath>` (TBD)
- `[slug]` - Ürün detay sayfaları için (`/urun/[slug]/page.tsx`) - **Sabit:** `/urun/[slug]` formatı korunacak
...
| `/[slug]` | `src/app/[slug]/page.tsx` | Page | `slug` (category) | Server Component | Direct DB (Drizzle: `getCategoryBySlug`, `getProductsCursor`, `db`, `categories`, `drizzle-orm`) | Public | Kategori sayfası, RESERVED_SLUGS kontrolü var (kanıt: lines 2-8, 13-15, 22-36, 47). **Not:** Kategori URL pattern'i TBD (karar sonra). Placeholder: `/<...categoryPath>` (TBD) |
...
| `/admin/returns` | `src/app/admin/returns/page.tsx` | Page | - | Unknown | Unknown | Protected (admin role) | İptal/İade (V1 minimal, sipariş detayından yönetilebilir) |
| `/admin/settings` | `src/app/admin/settings/page.tsx` | Page | - | Unknown | Unknown | Protected (admin role) | Ayarlar |
| `/admin/settings/undo` | `src/app/admin/settings/undo/page.tsx` | Page | - | Unknown | Unknown | Protected (admin role) | İşlemi Geri Al (log listesi + geri al, pagination zorunlu) |
| `/admin/users` | `src/app/admin/users/page.tsx` | Page | - | Unknown | Unknown | Protected (admin role) | Personel / Kullanıcılar (V2 — disabled, "Yakında" etiketi) |
| `/admin/export` | `src/app/admin/export/page.tsx` | Page | - | Unknown | Unknown | Protected (admin role) | Dışa Aktar (V2 — disabled, "Yakında" etiketi) |
...
- **Metadata:** `title: "ARTI OPTİK"`, `description: "ARTI OPTİK - Optik ve Gözlük"`
...
- **Links (V1):**
  ...
  - `/admin/returns` - İptal/İade (V1 minimal, sipariş detayından yönetilebilir)
...
**Ödeme/Kargo Notları (ARTI OPTİK V1):**
...
### 7.6 Admin Pagination Rules
**Zorunlu Kural:** Tüm admin liste sayfalarında pagination zorunludur. Sistem 1000+ kayıt varmış gibi düşünülmeli ve pagination implementasyonu yapılmalıdır.

**Pagination Zorunlu Sayfalar:**
- `/admin/orders` - Sipariş listesi
- `/admin/products` - Ürünler listesi
- `/admin/inventory` - Stok Yönetimi listesi
- `/admin/payments` - Ödemeler (PayTR) listesi
- `/admin/customers` - Müşteriler listesi (sondan başa sıralama)
- `/admin/settings/undo` - İşlemi Geri Al log listesi

**Not:** Bu kural, performans ve kullanıcı deneyimi için kritiktir. Tüm admin liste sayfaları cursor-based veya offset-based pagination kullanmalıdır.
...
- **Root Layout:** `metadata` export var (`title: "ARTI OPTİK"`, `description: "ARTI OPTİK - Optik ve Gözlük"`)
