...
Bu doküman, ARTI OPTİK Next projesinin veri getirme, cache/ISR, client vs server fetch, mutation/optimistic UI ve invalidation kurallarını tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) yanlış cache, yanlış fetch yeri ve gereksiz client state üretmesini engellemek ve tutarlı veri yönetimi sağlamak.
...
### 6.0 V1 Cache Hard Rules (Must Be Explicit)
...
- ANCAK gösterilen "stok" ve "fiyat" bilgisi fresh kaynaktan çekilmeli veya checkout'ta validate edilmelidir

**Payment Callbacks:**
- PayTR callback + payment status fetch hiçbir zaman cache'lenmemelidir

**Admin List Pages:**
- Admin list sayfaları her zaman paginated ve her zaman dynamic olmalıdır (asla pre-rendered olmamalı)

**Not:** Kategori URL mapping henüz TBD (To Be Determined) durumundadır. Cache kuralları final kategori route yapısını henüz varsaymamalıdır.
