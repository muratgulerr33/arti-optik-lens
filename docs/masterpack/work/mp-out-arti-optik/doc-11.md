...
Bu doküman, ARTI OPTİK V1 projesinin QA test matrisini, kritik akış test senaryolarını, erişilebilirlik kontrol listelerini ve release gate kriterlerini tanımlar. Amaç: Murti'nin "ChatGPT ile kontrol/test" rolünü standardize etmek ve her release öncesi tutarlı kalite kontrolü sağlamak.
...
- **Steps:**
  ...
  3. Ürün bulunursa satış oluşturulur
  4. Stok azalır
- **Expected:**
...
- **Steps:**
  ...
  3. Bir POS transaction undo edilir
- **Expected:**
...
- **Steps:**
  1. Online checkout akışında PayTR kullanılır
  2. PayTR callback işlenir
- **Expected:**
  - Online checkout uses PayTR; callback not cached; idempotency behavior test
- **Evidence:** PayTR integration, callback handling, idempotency implementation

### SMK-13: Admin Pagination
- **Steps:**
  1. Admin panelinde Orders listesine gidilir
  2. Products listesine gidilir
  3. Customers listesine gidilir
  4. Payments listesine gidilir
- **Expected:**
  - Orders/Products/Customers/Payments lists never load 1000 at once; page/pageSize or cursor works
- **Evidence:** Admin pagination implementation, list endpoints
...
### 9.4 Cache Rules (from doc-09)
- [ ] **Stock/price always fresh:** Stock ve fiyat bilgileri her zaman fresh olarak çekilir, cache'lenmez
- [ ] **Admin endpoints dynamic/no-store:** Admin ve POS endpoint'leri `dynamic` veya `no-store` cache policy kullanır
- [ ] **POS endpoints no-store:** POS endpoint'leri `no-store` cache policy kullanır

**Evidence:** `09.data-fetching-cache-rules.md` (cache rules), admin endpoints, POS endpoints
...
## 13. V2 Placeholder Test Section

### 13.1 AI Style Advisor (Gemini) - Feature Flag Behind
- **Status:** V2 feature, basic smoke tests only; no UI change in V1
- **Feature Flag:** AI Style Advisor feature flag arkasında aktif
- **Test Scope:**
  - [ ] Feature flag kapalıyken UI değişikliği yok (V1 davranışı korunur)
  - [ ] Feature flag açıkken AI Style Advisor temel smoke testleri çalışır
  - [ ] Gemini API entegrasyonu temel testleri (basic smoke tests only)
- **Note:** V1'de UI değişikliği olmamalı, sadece feature flag arkasında temel testler

**Evidence:** AI Style Advisor implementation, feature flag configuration, Gemini API integration

---

## 14. Exit Criteria (Release Gate)

Release için minimum kriterler. Tümü PASS olmalı.

### 14.1 Critical Checks
- [ ] **Tüm smoke testler PASS:** SMK-01 ile SMK-13 arası tüm akışlar çalışır
