...
- [ ] **Redeploy previous release (Kamatera VPS):**
  ...
  - Uygulama dizininde (`/var/www/arti-optik-next`) `git` ile önceki stabil commit/tag'e dön (hangi tag/commit kullanıldığı Unknown ise not düş)
  - Deploy dokümanındaki akışla tekrar build al (`npm ci`, `npm run build`)
  - PM2 ile process'i restart et (`pm2 restart arti-optik-next`)
- [ ] **Hotfix revert (self-hosted):**
  - `git revert` ile hatalı commit'i geri al
  - Deploy dokümanındaki standart deploy akışını tekrar çalıştır (`git pull`, `npm ci`, `npm run build`, `pm2 restart arti-optik-next`)
...
- [ ] `pm2 list` ile doğru process (`arti-optik-next`) doğrulandı
- [ ] Uygulama health endpoint / kritik sayfalar açılıyor (Home, PDP, Cart)
- [ ] Log kontrolü (`pm2 logs arti-optik-next --lines 200`)
